...
// --- 1. WEB AUDIO SYNTHESIZER ---
        ...
        function refreshCharacterSelectPreviews() {
            ...
            const p1Preview = spawnFighter(selections[1], gameMode === 'single' ? 0.8 : -0.8, true);
            const p2Preview = gameMode === 'single' ? null : spawnFighter(selections[2], 0.8, false);
        ...
        const selectSpotlight = new THREE.SpotLight(0xffffff, 4.0, 30, Math.PI / 3, 0.5, 1);
        selectSpotlight.position.set(0, 6, 8);
        selectSpotlight.target.position.set(0, 1.5, 3);
        selectSpotlight.castShadow = true;
        scene.add(selectSpotlight);
        scene.add(selectSpotlight.target);
        ...
        function updateCameraDirector() {
            ...
            if (cameraDirector.mode === 'select' && previewFighters.length > 0) {
                ...
                if (cameraDirector.mode === 'intro') {
                    ...
                    targetCamY = THREE.MathUtils.lerp(2.15, 3.25, shotProgress);
                    targetCamZ = THREE.MathUtils.lerp(4.7, 7.35, shotProgress);
                    lookTarget = new THREE.Vector3(
                        focus.mesh.position.x,
                        THREE.MathUtils.lerp(1.7, 1.05, shotProgress),
                        0.28
                    );

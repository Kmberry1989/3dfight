...
This project uses **PeerJS** to facilitate peer-to-peer (P2P) online multiplayer without requiring a dedicated backend game server.

## How It Works
1. **Signaling Server:** When a player opens the Online Multiplayer lobby, the game connects to the public PeerJS signaling server. This server's only job is to help two browsers find each other; it does not host the game logic.
